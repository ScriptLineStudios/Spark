...
static PyObject *getFps(PyObject *self) { return PyLong_FromLong(FPS); }
...
static PyObject *createRect(PyObject *self, PyObject *args) {
  ...
  const char *vertexShaderSource = GLSL(layout(location = 0) in vec3 aPos;
  ...
  const char *fragmentShaderSource =
      GLSL(out vec4 FragColor; in vec3 outColor;
  ...
  unsigned char *bytes =
      stbi_load(textureLocation, &imgWidth, &imgHeight, &colorChannels, 0);
...
static PyObject *renderRect(PyObject *self, PyObject *args) {
  ...
  GLfloat verticies[] = {renderX,
                         renderY + (size / windowY),
                         0.0f,
                         colorR,
                         colorG,
                         colorB,
...
static PyMethodDef myMethods[] = {
    ...
    {"render_rect", (PyCFunction)renderRect, METH_VARARGS,
     "Loads a new rect into memory"},
    {"create_rect", (PyCFunction)createRect, METH_VARARGS,
     "Loads a new rect into memory"},
    {"get_fps", (PyCFunction)getFps, METH_NOARGS,
     "Loads a new rect into memory"},
    {"update", (PyCFunction)render, METH_NOARGS, "Render"},
    {"set_title", (PyCFunction)setTitle, METH_VARARGS,
     "Loads a new rect into memory"},
    {"key_is_pressed", (PyCFunction)keyIsPressed, METH_VARARGS,
     "Checks key pressed"},
    //{"load_texture", (PyCFunction)load_texture, METH_VARARGS, "Checks key
    // pressed"},
    {NULL, NULL, 0, NULL}};

static struct PyModuleDef spark = {PyModuleDef_HEAD_INIT, "Spark", "Base", -1,
                                   myMethods};

PyMODINIT_FUNC PyInit_spark(void) { return PyModule_Create(&spark); }